...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//별도의 VALUE를 지정하지 않았기에 체크가 됐다는 ON 데이터가 전달됨 (해제 NULL)
		//만약 체크가 돼 넘어온다면 해당 이이디값을 쿠키에 달망서 사용자가 로그아웃을 해도
		//해당 아이디값이 입력되어 있도록 처리하기
		/*
		 * 쿠키
		 * 서버에 저장되지 않고 클라이언트에 저장되는 문자열 기반 저장소
		 * key와 value 로 이루어진 데이터 파일
		 * 사용자가 요청하지 않아도 브라우저가 request시 header에 추가하여 서버에 전송됨
		 * 서버와 뷰에서 모두 접근가능
		 * 만료시간 설정가능, 문자열만 저장가능
		 * 
		 * */
		...
		if(saveId != null) {
			...
			//쿠키의 수명을 지정
			cookie.setMaxAge(60*60*24); //초단위 ( 하루: 60*60*24)
			//response객체에 추가하여 응답되도록
